...
let $annots := collection('/db/apps/bargheer-edition')//mei:annot[matches(@plist, $uri)] | $mei//mei:annot

return concat('{categories: [',
        string-join(
            for $category in local:getDistinctCategories($annots)
            return
                concat('{id:"', $category, '",name:"', (collection('/db/apps/bargheer-edition')//id($category))[1]/mei:name/text(),'"}')
        , ','),
        '], priorities: [',
        string-join(
            for $priority in local:getDistinctPriorities($annots)
            return
                concat('{id:"', $priority, '",name:"', (collection('/db/apps/bargheer-edition')//id($priority))[1]/mei:name/text(),'"}')
        , ','),
        ']}')